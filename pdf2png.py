...
def convert_job( page ):
	convert_args = [ 'convert', '-units', 'PixelsPerInch', '-density', '400', '{}[{}]'.format( filename, page - 1 ), '-channel', 'RGBA', '-fill', 'white', '-opaque', 'none', '{}_{:03d}.png'.format( basename, page ) ]
...
if __name__ == '__main__':
	...
	if argc > 0 and argc < 3:
		if os.path.exists(sys.argv[1]):
			...
			print( '\r{:3d} of {:3d} pages converted.'.format( finished_jobs, pages ), end='' )

			while finished_jobs < pages:
				if next_job <= pages and len( children ) < max_children:
					children.append( convert_job( next_job ) )
					# print( 'There are currently {} processes running'.format( len( children ) ) )
					next_job += 1
				else:
